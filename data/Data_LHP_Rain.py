...
#     parser.add_argument('--dataroot', type=str, default=r'D:\DataSets\Derain_change\LHP-Rain_New')
#     return parser.parse_args()

# cfg=parse_args()

"""Train RealRain1K_H data Augmentation"""
class Train_LHP_Rain(Dataset):
    def __init__(self, opt, patch_size):
        super(Train_LHP_Rain, self).__init__()
        # self.root_dir = os.path.join(opt['data_root'], 'train')
        self.root_dir = opt['data_root']

        self.rain_path = os.path.join(self.root_dir,'input')
        self.norain_path = os.path.join(self.root_dir,'gt')
...
class Test_LHP_Rain(Dataset):
    def __init__(self, opt):
        super(Test_LHP_Rain, self).__init__()
...
class Val_LHP_Rain(Dataset):
    def __init__(self, opt):
        super(Val_LHP_Rain, self).__init__()
        # self.root_dir = os.path.join(opt['data_root'], 'test')
        self.root_dir = opt['data_root']
...
# 用来测试 full size
class Test_LHP_Rain_FullSize(Dataset):
    def __init__(self, opt):
        super(Test_LHP_Rain_FullSize, self).__init__()
        # self.root_dir = os.path.join(opt['data_root'], 'test')
        self.root_dir = opt['data_root']
...
#     traindataset = Train_LHP_Rain(patch_size=128)
