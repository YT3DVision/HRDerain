        ...
        if self.semi_conv:
            if self.comp_feat_upsample:
                if self.use_high_pass:
                    ...
                    # print("compressed_hr_feat",compressed_hr_feat.shape)
                    # print("mask_hr_hr_feat",mask_hr_hr_feat.shape)
                    # print("mask_hr_init",mask_hr_init.shape)
                    ...
                    mask_lr_lr_feat_lr = self.content_encoder(compressed_lr_feat) #从hr_feat得到另一部分初始低通滤波特征 ??? lr
                    ...
                    # print("mask_lr_lr_feat_lr",mask_lr_lr_feat_lr.shape)
                    # print("mask_lr_init",mask_lr_init.shape)
                    # print("mask_lr_hr_feat",mask_lr_hr_feat.shape)
                    ...
                    # print("carafe",carafe(self.content_encoder2(compressed_lr_feat), mask_lr_init, self.lowpass_kernel, self.up_group, 2).shape)
                    # print("mask_lr_init",mask_lr_init.shape)
                    # print("mask_hr_lr_feat",mask_hr_lr_feat.shape)
...
#         self.model = FreqFusion(hr_channels=32, lr_channels=32)
...
#         _, hr_feat, lr_feat = self.model(hr_feat=hr, lr_feat=lr)

#         hr_feat = self.conv2(hr_feat)
...
# model = pernet().to(device)

# # 读取图片并进行预处理
# image_path = 'D:/PycharmProjects/loproj/HRderain/Datasets/RealRain1K_L/validation/input/3.png'
# image_path2 = 'D:/PycharmProjects/loproj/HRderain/Datasets/RealRain1K_L/validation/target/3.png'
...
#     loss = criterion(out, image2)

#     optimizer.zero_grad()
#     loss.backward()
#     optimizer.step()

# # 将处理后的张量的每个通道转换回图片并保存
# to_pil = ToPILImage()
# output_dir = 'output_images'
# os.makedirs(output_dir, exist_ok=True)

# for i in range(out.shape[1]):  # 遍历每个通道
#     channel_image = out[0, i, :, :].cpu().detach()  # 提取单个通道
#     pil_image = to_pil(channel_image)  # 转换为 PIL 图像
#     pil_image.save(os.path.join(output_dir, f'channel_{i}.png'))  # 保存图片

# print(f"Saved {out.shape[1]} channel images to {output_dir}")
...
# out = unnormalize(out)
# rgb_image = to_pil(out)
# rgb_image.save(os.path.join(output_dir, 'output_rgb.png'))

# print(f"Saved RGB image to {output_dir}")




# # 将图片输入模型
# hr_feat, lr_feat = model(image)

# # 将处理后的张量转换回图片
# to_pil = ToPILImage()
# hr_image = to_pil(hr_feat.squeeze(0).cpu().detach())
# lr_image = to_pil(lr_feat.squeeze(0).cpu().detach())

# # 保存图片
# hr_image.save('hr_image.jpg')
# lr_image.save('lr_image.jpg')



from pynvml import nvmlDeviceGetHandleByIndex, nvmlInit, nvmlDeviceGetCount, nvmlDeviceGetMemoryInfo, nvmlDeviceGetName, nvmlDeviceGetTemperature, nvmlShutdown
...
if __name__ == '__main__':
    ...
    W = 1080
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    hr_feat = torch.rand(1, c, H, W).to(device)
    lr_feat = torch.rand(1, c, int(H/2), int(W/2)).to(device)
    model = simplefreqfusion(hr_channels=c, lr_channels=c).to(device)
    _, hr_feat, lr_feat = model(hr_feat=hr_feat, lr_feat=lr_feat)
